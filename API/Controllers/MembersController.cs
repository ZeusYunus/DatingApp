...
namespace API.Controllers
{
    ...
    public class MembersController(IUnitOfWork unitOfWork, IPhotoService photoService) : BaseApiController
    {
        ...
        // The <Photo> is what we sent back to the client after the request is done
        // Task<> is an asynchronous operation that returns a value
        // ActionResult returns status codes back to the client like 200, 404 and 401
        // IReadOnlyList specifies that the returned data is a read-only list of Photo
        [HttpGet("{id}/photos")]
        ...
        public async Task<ActionResult> UpdateMember(MemberUpdateDto memberUpdateDto)
        {
            ...
            unitOfWork.MemberRepository.Update(member); //options
        ...
        public async Task<ActionResult> SetMainPhoto(int photoId)
        {
            ...
            if (member == null) BadRequest("Cannot get member from token");
